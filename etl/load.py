...
Constraints required before first run:
...
def resolve_input_files() -> tuple[Path, Path, Path]:
    ...
    return (
        latest(f"donors_{prefix}.parquet"),
        latest(f"committees_{prefix}.parquet"),
        latest(f"edges_{prefix}.parquet"),
    )
...
def main():
    ...
    city_slug     = donors_file.stem.split("_")[1]
    ...
    with driver.session(database=NEO4J_DATABASE) as session:
        ...
        written_committees = write_batches(
            session, df_to_rows(committees_df), MERGE_COMMITTEE, "committees", logger)
        log(logger, "info", "load", "committees_complete",
            "Committee nodes written", records=written_committees)

        print("\n  Writing Donor nodes...")
        written_donors = write_batches(
            session, df_to_rows(donors_df), MERGE_DONOR, "donors", logger)
        log(logger, "info", "load", "donors_complete",
            "Donor nodes written", records=written_donors)

        aura_donors     = session.run("MATCH (d:Donor) RETURN count(d) AS n").single()["n"]
        aura_committees = session.run("MATCH (c:Committee) RETURN count(c) AS n").single()["n"]
        ...
        written_edges = write_batches(
            session, df_to_rows(edges_df), MERGE_EDGE, "edges", logger)
        log(logger, "info", "load", "edges_complete",
            "CONTRIBUTED_TO relationships written", records=written_edges)

        aura_edges = session.run(
            "MATCH ()-[r:CONTRIBUTED_TO]->() RETURN count(r) AS n").single()["n"]
